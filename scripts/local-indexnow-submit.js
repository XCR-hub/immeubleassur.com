...
const sitemapPath = resolve(env("INDEXNOW_SITEMAP", join("public", "sitemap.xml")));
...
async function run() {
  ...
  const rows = sitemapRows(readFileSync(sitemapPath, "utf8"));
  ...
  const base = { generated_at: new Date().toISOString(), provider: "indexnow", endpoint: "https://api.indexnow.org/IndexNow", site, key_location: keyLocation, key_publicly_verifiable: true, sitemap_urls: rows.length, first_run: firstRun, changed_urls: changed.length, submitted_urls: 0, citation_or_ranking_guaranteed: false };
