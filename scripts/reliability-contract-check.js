...
const checks=[
  ...
  ["production-monitor-directly-enforces-ai-discoverability",runtime.includes('runStep("ai_discoverability_monitor"')&&monitor.includes('inspectJsonRuntime("ai_discoverability", aiDiscoverabilityPath, 90')&&monitor.includes('report.status === "ready"')&&monitor.includes("report.missing.length === 0")&&monitor.includes("report.public_editorial_metadata?.sanitized === true")&&monitor.includes("report.public_editorial_metadata?.ai_generated === false")&&monitor.includes("report.policies?.citation_guaranteed === false")&&monitor.includes('privacy: report.crawler_observation?.privacy || "unavailable"')],
  ...
  ["monitor-verifies-last-locked-deployment",monitor.includes("inspectProductionCheckout(productionCheckoutUpdatePath)")&&monitor.includes("revision_matches_runtime")&&monitor.includes("runtime_revision_verified")&&monitor.includes("served_revision_matches")&&monitor.includes("LOCAL_PRODUCTION_CHECKOUT_UPDATE_MAX_AGE_MINUTES")&&monitor.includes("ageMinutes <= maxAgeMinutes")&&monitor.includes("requiredSafeguards.every")&&monitor.includes("revisionAfter === sourceRevision")],
  ...
  ["monitor-verifies-every-editorial-artifact-over-http",monitor.includes('inspectJsonRuntime("editorial_publication_http"')&&monitor.includes("editorialPublicationSmokePath")&&monitor.includes("report.checks.length >= 8")&&monitor.includes("report.checks.every((item) => item.ok === true)")&&monitor.includes("failed_artifacts")&&monitor.includes("http_failures")&&monitor.includes('item.local_sha256 && item.remote_sha256 && item.remote_sha256 !== item.local_sha256')&&monitor.includes('"veille", "newsletter", "news", "faq", "villes", "llms", "sitemap"')],
  ...
  ["monitor-covers-imap-review-backlog",monitor.includes("inspectImapInbox")&&monitor.includes("imap-review-backlog-alerted")&&monitor.includes("alert_delivery_verified")&&monitor.includes("pending_unmatched")&&monitor.includes("smtp_roundtrip_verified")&&monitor.includes("smtp_roundtrip_receipts")],
