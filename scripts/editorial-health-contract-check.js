...
const checks = [
  ...
  ["business-coverage-gap-operational-only", monitor.includes("const missingCoverageDimensions = operationalCycle")],
  ["business-freshness-gaps-monitored", monitor.includes("freshnessCoverageGaps") && monitor.includes('freshness_status === "freshness-gaps-detected"') && monitor.includes("missing_fresh_dimensions")],
  ["freshness-gaps-identified-in-alert", monitor.includes("freshness_gaps: persistentCoverageGaps.filter")],
  ...
  ["admin-api-exposes-sanitized-freshness-gaps", api.includes("normalizedFreshnessGaps") && api.includes("business_freshness_status:") && api.includes("freshness_gap: normalizedFreshnessGaps.has(dimension)")],
  ["admin-api-normalizes-coverage-dimensions", api.includes('.toLowerCase().replace(/[^a-z0-9-]/g, "").slice(0, 40)')],
  ["admin-dashboard-shows-editorial-health", admin.includes("editorialHealthStatusLabel") && admin.includes('label:"Veille editoriale"')],
  ["admin-dashboard-shows-coverage-before-alert", admin.includes('e.coverage_gaps?.length?"A surveiller"') && admin.includes("couverture a renforcer:") && admin.includes("coverage_gap_alert_cycles")],
  ["admin-dashboard-labels-freshness-gap", admin.includes('e.freshness_gap?" fraicheur":""') && admin.includes('(fraicheur)')],
