...
const report={generated_at:new Date().toISOString(),status:failed.length?"failed":"passed",success:failed.length===0,origin,checks,summary:{ok:checks.length-failed.length,failed:failed.length},safeguards:["live-header-verification","admin-no-store","admin-no-referrer","anonymous-admin-denial","minimal-health-response","security-contact"]};
mkdirSync(dirname(out),{recursive:true}); writeFileSync(out,`${JSON.stringify(report,null,2)}\n`,`utf8`); console.log(`Security surface monitor: ${report.status} (${report.summary.ok}/${checks.length}).`); if(failed.length)process.exit(1);