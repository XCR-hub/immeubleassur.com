...
const FALLBACK_EDITORIAL_INTRO = "La veille ImmeubleAssur transforme les signaux publics en actions de verification pour les immeubles, coproprietes, SCI et proprietaires non occupants.";
const FALLBACK_EDITORIAL_AUDIT = "Avant de demander un devis, relire le contrat actuel, l'appel de prime, les franchises, les exclusions et l'historique sinistres reste le meilleur levier de qualite.";

function editorialContext(source = "editorial") {
  const edition = String(source).match(/veille-assurance-immeuble-(\d{4})-(\d{2})-(\d{2})/);
  if (edition) return `l'edition du ${edition[3]}/${edition[2]}/${edition[1]}`;
  return ({ "veille-page": "la veille permanente", "newsletter-page": "la page newsletter", "hub-injection": "le hub editorial" })[source] || "ce parcours de veille";
}

function contextualizeEditorialCopy(value, source) {
  const context = editorialContext(source);
  return String(value || "")
    .replace(FALLBACK_EDITORIAL_INTRO, `Pour ${context}, les signaux publics sont transformes en controles utiles aux immeubles, coproprietes, SCI et proprietaires non occupants.`)
    .replace(FALLBACK_EDITORIAL_AUDIT, `Pour ${context}, relire contrat, appel de prime, franchises, exclusions et historique sinistres prepare un devis plus fiable.`);
}

function editorialReadinessCopy(source) {
  return `Pour ${editorialContext(source)}, relier contrat, echeance, sinistres et travaux rend le signal actionnable et prepare l'echange avec le courtier.`;
}

function readinessBlock(source = "editorial") {
  const id = `editorial-readiness-${hash(source, 8)}`;
  return `<section class="band readiness-band" aria-labelledby="${attr(id)}"><div class="readiness-shell" data-readiness data-editorial-readiness="${attr(source)}"><div class="readiness-copy"><p class="eyebrow dark">Dossier pret assureur</p><h2 id="${attr(id)}">Verifier si le signal de veille justifie un audit.</h2><p class="large-copy">${esc(editorialReadinessCopy(source))}</p></div><div class="readiness-panel"><div class="readiness-meter"><span class="readiness-label">Niveau de preparation</span><strong class="readiness-score" data-readiness-score>20%</strong><div class="readiness-bar" aria-hidden="true"><span data-readiness-bar></span></div></div><div class="readiness-checks" aria-label="Pieces disponibles"><label><input type="checkbox" data-readiness-item data-points="22" data-label="contrat actuel" value="contrat-actuel">Contrat actuel</label><label><input type="checkbox" data-readiness-item data-points="20" data-label="appel de prime" value="appel-prime">Appel de prime</label><label><input type="checkbox" data-readiness-item data-points="18" data-label="sinistres 36 mois" value="sinistres-36-mois">Sinistres 36 mois</label><label><input type="checkbox" data-readiness-item data-points="16" data-label="nombre de lots" value="nombre-lots">Nombre de lots</label><label><input type="checkbox" data-readiness-item data-points="14" data-label="echeance" value="echeance">Echeance</label><label><input type="checkbox" data-readiness-item data-points="10" data-label="travaux prevus" value="travaux-prevus">Travaux prevus</label></div><p class="readiness-next" data-readiness-next>Selectionnez les pieces disponibles pour prioriser la prochaine action.</p><a class="button primary readiness-cta" data-track="editorial-readiness-devis" href="/devis-assurance-immeuble?intent=veille&readiness=1">Preparer mon audit</a></div></div></section>`;
}
...
function injectIssueBacklog() {
  ...
  for (const name of readdirSync(dir)) {
    if (/^veille-assurance-immeuble-\d{4}-\d{2}-\d{2}\.html$/.test(name)) {
      ...
      const source = name.replace(/\.html$/, "");
      const html = read(file);
      const contextualized = contextualizeEditorialCopy(html, source);
      if (!RUNTIME_ONLY && contextualized !== html) write(file, contextualized);
      if (injectBlock(file, "editorial-intent-exits", intentExitBlock(source))) changed += 1;
    }
  }
  return changed;
}
