...
const checks = [
  ...
  ["admin-and-api-remain-protected", (robots.match(/Disallow: \/admin$/gm) || []).length >= 3 && !robots.includes("Disallow: /admin.html") && (robots.match(/Disallow: \/api\//g) || []).length >= 3],
  ...
  ["runtime-sanitizer-retries-windows-sharing-violation", sanitizer.includes('["EPERM", "EACCES"]') && sanitizer.includes("attempt <= 20") && sanitizer.includes("Atomics.wait") && sanitizer.includes("rmSync(temporaryPath, { force: true })") && sanitizer.includes('[IO.File]::Replace') && sanitizer.includes('$attempt -le 60') && sanitizer.includes('Start-Sleep -Milliseconds 500') && sanitizer.includes("timeout: 35000") && sanitizer.includes('spawnSync("powershell.exe"') && sanitizer.includes("rmSync(backupPath, { force: true })")],
