export function classifyWorkflowHealth(scheduled, recovery, now = Date.now(), maxAgeHours = 36) {
  ...
  const recoveryHealthy = recovery?.event === "workflow_dispatch" && recovery?.status === "completed" && recovery?.conclusion === "success" && Number.isFinite(recoveryAt) && recoveryAt > scheduledAt && recoveryAgeHours <= maxAgeHours && recovery?.head_sha;
  return { healthy: Boolean(scheduledHealthy || recoveryHealthy), status: scheduledHealthy ? "healthy" : recoveryHealthy ? "recovered-awaiting-schedule" : ageHours > maxAgeHours ? "stale" : "failed", scheduled_age_hours: Number.isFinite(ageHours) ? Number(ageHours.toFixed(1)) : null, recovery_age_hours: Number.isFinite(recoveryAgeHours) ? Number(recoveryAgeHours.toFixed(1)) : null, scheduled_success: Boolean(scheduledHealthy), recovery_verified: Boolean(recoveryHealthy) };
}
