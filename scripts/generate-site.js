...
function leadForm(defaults = {}) {
  ...
  const priorityContact = defaults.priorityContact ? `<div class="quote-priority-contact" data-quote-priority-contact><span><strong>Dossier urgent ?</strong><small>Sinistre, resiliation, refus ou echeance proche : appelez pendant que vous gardez le formulaire sous les yeux.</small></span><a class="button secondary" data-track="quote-priority-phone" href="tel:${PHONE_HREF}">${PHONE}</a></div>` : "";
  return `
    ...
    <main class="plain-main">
      ...
      <section class="admin-table-wrap" aria-label="Derniers leads">
        <table class="admin-table">
          <thead><tr><th>Date</th><th>Reference</th><th>Contact</th><th>Priorite</th><th>Profil</th><th>Bien</th><th>Ville</th><th>Besoin</th><th>Origine</th><th>Statut</th><th>Score</th><th>Valeur</th><th>Action</th><th>Message</th></tr></thead>
          <tbody id="leads-body"><tr><td colspan="14">Aucun chargement effectue.</td></tr></tbody>
        </table>
      </section>
      <!-- sales-admin:start -->
...
function writeStatic() {
  ...
  write("faq", faqPage());
  // Caddy serves directory requests from index.html. Keep the extensionless
  // hub URLs aligned with the canonical .html pages generated above.
  for (const slug of ["blog", "faq"]) {
    const source = join(OUT, `${slug}.html`);
    const directory = join(OUT, slug);
    mkdirSync(directory, { recursive: true });
    copyFileSync(source, join(directory, "index.html"));
  }
  write("contact", contactPage());
  write("merci", simplePage("merci", "Votre demande est enregistree", "Confirmation demande ImmeubleAssur.", `<p>Un conseiller ImmeubleAssur vous rappelle rapidement pour qualifier le risque et preparer la suite.</p><p><a class="button primary" href="/">Retour accueil</a></p>`).replace('<meta name="robots" content="index, follow, max-image-preview:large" />', '<meta name="robots" content="noindex, nofollow" />'));
