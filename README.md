...
Le watchdog Node `scripts/local-site-watchdog.js` est utilise par la tache planifiee Windows. `scripts/install-local-site-watchdog-task.ps1` installe ou repare cette tache sous `SYSTEM`, au demarrage puis toutes les 5 minutes, avec instance unique; `npm run production:watchdog:task:contract` verifie ce contrat sans modifier Windows.
